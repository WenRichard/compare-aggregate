...
def main(args):
    ...
    train_corpus = list(zip(train_qids, train_q, train_aids, train_ap, train_labels))


    val_qids, val_q, val_aids, val_ap, labels = zip(*val_corpus)
    val_q = padding(val_q, max_q_length)
    val_ap = padding(val_ap, max_a_length)
    val_corpus = list(zip(val_qids, val_q, val_aids, val_ap, labels))
    ...
    test_corpus = list(zip(test_qids, test_q, test_aids, test_ap, labels))
    ...
    if args.train:
        train(deepcopy(train_corpus), config, test_corpus, deepcopy(train_corpus))
    elif args.test:
